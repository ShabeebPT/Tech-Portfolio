          ...
          {projects.map((project: any, index: number) => (
            ...
            >
              ...
              <div className="w-full lg:w-1/2 relative group">
                ...
                <div className="relative rounded-2xl overflow-hidden glass border border-text-primary/10 shadow-2xl bg-card">
                  ...
                  <div className="aspect-[16/10] w-full bg-gradient-to-br from-gray-800 to-gray-900 flex items-center justify-center relative overflow-hidden">
                    ...
                    />
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 [.fallback-img_&]:opacity-100 transition-opacity bg-text-primary/40">
                      <p className="text-text-primary font-mono text-2xl font-bold opacity-30">{project.title}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className={`w-full lg:w-1/2 flex flex-col ${index % 2 !== 0 ? 'lg:items-start lg:text-left' : 'lg:items-end lg:text-right'}`}>
                ...
                <h3 className="text-3xl md:text-4xl font-bold text-text-primary mb-6 hover:text-primary transition-colors cursor-pointer">
                  {project.title}
                </h3>
                ...
                <ul className={`flex flex-wrap gap-3 mb-8 ${index % 2 !== 0 ? 'justify-start' : 'justify-end'}`}>
                  {project.tech.map((tech: string) => (
                    <li key={tech} className="text-sm font-mono text-text-secondary px-3 py-1 bg-text-primary/5 rounded-full border border-text-primary/5">
                      {tech}
                    </li>
                  ))}
                </ul>
                ...
                <div className="flex gap-4">
                  {project.github && (
                    <a href={project.github} target="_blank" rel="noopener noreferrer" className="text-text-secondary hover:text-text-primary transition-colors" aria-label="GitHub Repository">
                      <Code2 size={20} />
                    </a>
                  )}
                  {project.demo && (
                    <a href={project.demo} target="_blank" rel="noopener noreferrer" className="text-text-secondary hover:text-text-primary transition-colors" aria-label="Live Demo">
